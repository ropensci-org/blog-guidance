...
If you want to use mathematical formula,
  
* install the package [katex](https://docs.ropensci.org/katex/).
* add `math: true` to your post YAML.
* for all math you'll need to double the backslashes. All "\" become "\\".
* for inline math (`$1+1$`), use no dollar sign but instead.
  
```r
`r katex::katex_html("1 + 1", displayMode = FALSE)`
```

* for displayed math (starting and ending with `$$`)
