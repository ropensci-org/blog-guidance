...
This chapter explains the technical details of submitting a blog post or tech note for publication on the rOpenSci website. All in all, your post will be an **`(R) Markdown` file** with some tweaks due to the use of **Hugo** for our website, accompanied by your author metadata as Markdown and optional images, and placed at the right place in **our website folder structure**; and the general submission and review process uses **GitHub Pull Request infrastructure**.

This chapter explains the [local post drafting](#forkcreate), [where to find templates](#templates), [how to use the templates](#usetemplates), [what our style guide is](#styleguide), [how to add your author metadata](#createauthorfile), [how to check your post](#presubchecks), [how to submit it for our review](#submitpost).

Furthermore, this chapter includes advice on commonly used things like adding [an image](#addimage), [a citation](#addcitation), or [embedding a tweet](#addtweet) in case you need them.
```
...
This chapter links templates for posts and checklists that you can also find in the Appendix.
...
Your post source should be either `/content/blog/YYYY-MM-DD-slug/index.md` (prepared in Markdown directly) or `/content/blog/YYYY-MM-DD-slug/index.Rmd` (using R Markdown).

Prepare your post in whichever way is most comfortable for you in your favorite editor in R Markdown **or** Markdown by starting with templates we provide.
Get the templates from this book or [from the roblog package](#roblogtemplates).
...
[R Markdown template](#templatermd) to be saved as `/content/blog/YYYY-MM-DD-slug/index.Rmd`
...
You could get templates from the [roblog helper package](https://docs.ropensci.org/roblog/) created for this purpose.
Installing roblog allows you to use RStudio Addins to get templates and to run some pre-submission checks.
...
- A blog post about a package that has passed software peer review. Compare [raw markdown](https://raw.githubusercontent.com/ropensci/roweb2/master/content/blog/2019-10-21-rmangal.md) with the [live post](https://ropensci.org/blog/2019/10/21/rmangal/)

- A technote. Compare [raw markdown](https://raw.githubusercontent.com/ropensci/roweb2/master/content/technotes/2018-10-06-av-release.md) with the [live tech note](https://ropensci.org/technotes/2018/10/06/av-release/)
...
### To add a tweet {#addtweet}

Use a Hugo shortcode to embed a tweet using its ID e.g. `{{< tweet 1138216112808529920 >}}`. In R Markdown, shortcodes need to be html escaped, refer to [the template](https://github.com/ropensci-org/blog-guidance/blob/master/templates/post-template.Rmd) for an example.

### Don't use html widgets

We've decided against featuring html widgets (`DT`, `leaflet`, etc.) in posts because

* their JS and CSS could clash with our website's JS and CSS;

* using an iframe to embed them, setting the correct width/height is tricky.

If you really want to show an html widget, consider publishing it elsewhere and linking it from the blog post.
...
Delete `/content/blog/YYYY-MM-DD-slug/index.html` if present.
...
Do not commit `index.html` but do commit `index.Rmd` and `index.md`.

* Open a [**draft** pull request](https://help.github.com/en/github/collaborating-with-issues-and-pull-requests/about-pull-requests#draft-pull-requests) from your fork ([using the web interface]((https://help.github.com/en/github/collaborating-with-issues-and-pull-requests/creating-a-pull-request-from-a-fork)), or [`usethis::pr_push()`](https://usethis.r-lib.org/articles/articles/pr-functions.html#submit-pull-request))

* Mark the PR as ready for review at least one week prior to the planned publication date to allow time for review. A review from blog editor(s) will be automatically requested by GitHub.

* If you opened a PR instead of a **draft** PR, the PR can't go back to a draft stage but no problem, only make sure to tag `@ropensci/blog-editors` once to tell them the PR isn't ready for review, and once later to tell them the PR is really ready for review.

From the PR, even when it's still a draft PR, Netlify will start building the new version of the site within seconds and you can preview your changes to make sure everything looks as intended.
