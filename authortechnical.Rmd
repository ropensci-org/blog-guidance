...
If you need to add an alert box at the beginning of the post, like for instance the link to the blog post in Spanish in this [post](https://ropensci.org/blog/2022/11/23/r-universe-stars-1-en/), use the `preface` field with the Markdown syntax.
...
- For a post in say Spanish only, use the same folder structure as for an English post but call the post file `index.es.md`.
...
- If you are writing the first blog entry for a language (yay!), please ask blog editors to notify the website team so we can add translation for the blog infrastructure (such as "Share this page" and similar phrases).
...
- **Language**
  ...
  - Avoid Latin phrases such as "e.g." (for instance) or "i.e." (that is to say).
